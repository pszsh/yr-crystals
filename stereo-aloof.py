...
def calculate_spectrum_frame(segment, samplerate):
    ...
    custom_bins = [9.72, 12.25, 15.43, 19.45, 24.5, 30.87,
                    36.71, 43.65, 51.91, 61.74, 73.42, 87.31,
                    103.83, 123.47, 146.83, 174.61, 207.65, 246.94,
                    293.66, 349.23, 415.3, 493.88, 587.33, 698.46,
                    830.61, 987.77, 1108.73, 1318.51, 1567.98, 1864.66,
                    2349.32, 2959.96, 3729.31, 4698.64, 5919.91,
                    7458.62, 9397.27, 11839.82, 14917.24, 18794.55,
                    23679.64, 29834.48]
...
# ========================================================================
def animate_lufs_plot(lufs_vals_channels, file_path, sample_stride, samplerate, audio_data, band_lufs_channels, frame_rate=60, frame_size=4096, num_processes=None): # Added num_processes
    """
    ...
    fig, ax = plt.subplots(figsize=(10, 7), facecolor='black') # Set figure facecolor to black
    ax.set_xscale('log')
...
if __name__ == "__main__":
    ...
    parser.add_argument("-f", "--framerate", type=float, default=30.0, help="Frame rate for visualization and video (default: 30.0).")
    parser.add_argument("-s", "--framesize", type=int, default=8192, help="Frame size (sample size) for processing (default: 8192).") # Changed to framesize
...
we need to add conccurrent.futures support for the playback part of the process, the buffering runs wonderfully, but extending it to playback would not be wise because we don't want lowlevel on something like playback. it's why concurrency exists